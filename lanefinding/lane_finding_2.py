...
def pipeline(image):
    ...
    undistort_display = np.copy(undistort)
    ...
    left_fit_new, right_fit_new, left_lane_inds, right_lane_inds = average_recent_fits(image_cmb_color, left_fit,
                                                                                       right_fit)

    # Start averaging
    left_fit_recent.append(left_fit_new)
    left_fit = np.mean(left_fit_recent, 0)
    right_fit_recent.append(right_fit_new)
    right_fit = np.mean(right_fit_recent, 0)

    image_lanes = draw_lane(undistort_display, image_cmb_color, left_fit, right_fit, Minv_warp)
...
left_fit_recent = deque(maxlen=5)
right_fit_recent = deque(maxlen=5)
